...
module Rubinius
  module AST
    ...
    class DynamicString < StringLiteral
      ...
      # String instance.
      #
      # If String.interpolate_join(...) is defined, the generated bytecode 
      # includes a call to that method to join the components of the dynamic
      # string.
      def bytecode(g)
        pos(g)

        # generate a list of things to be joined, removing empty string literals
        components = [StringLiteral.new(@line, @string)] + @array
        string_literals = 0
        components = components.select { |s|
          case x = s
          when StringLiteral
            if x.string.empty?
              false
        ...
        # optimize when there are less than two elements to be joined
        if components.empty?
          g.push_literal ""
          g.string_dup
          return
        elsif components.size == 1
          if string_literals == 1
            g.push_literal components.first.string
            g.string_dup
          else
            components.first.bytecode(g)
            g.string_build 1
          end
          return
        end

        # prepare for a list of arguments
        if String.respond_to? :interpolate_join
          interpolate_join = g.find_literal :interpolate_join
          g.push_const :String
        else
          interpolate_join = nil
        end

        # bytecode that pushes the arguments onto the stack
        ...
        # perform the join, using interpolate_join(...) if available
        if interpolate_join
          g.send_stack interpolate_join, components.size
        else
          g.string_build components.size
        end
      end
